...
// arquivo
$oDBase->query("SELECT id, upag, tipo, maquina, arquivo FROM relatorios_arquivos WHERE upag = '$upag' AND tipo = 'recesso' AND data_arquivo='$data_hoje' ORDER BY upag, arquivo DESC ");
$nRowsArq = $oDBase->num_rows();
$oArquivo = $oDBase->fetch_object();
$nId      = $oArquivo->id;

// define um nome para o arquivo PDF
$arquivo_pesquisar = 'compensacao_recesso_nao_realizada_' . $upag . "-" . date("Ymd");
...
$cabecalho .= "          <td colspan='5' align='center'>";
...
//$cabecalho .= "          <td width='15%' align='center' class='tahomaSize_1_BP'><b>C&Oacute;DIGO SIAPECAD</b></td>";
$cabecalho .= "          <td width='15%' align='center' class='tahomaSize_1_BP'><b>C&Oacute;DIGO</b></td>";
$cabecalho .= "          <td width='16%' align='center' class='tahomaSize_1_BP'><b>HORAS</b></td>";
$cabecalho .= "        </tr>";
...
if ($tam > 0)
{
    ...
    for ($x = 0; $x < $tam; $x++)
    {
        ...
        $idInner .= "<td align='left'  >" . tratarHTML($aDadosEncontrados[$x][1]) . "</td>";
...
if ($nRowsArq == 0)
{
    $oDBase->query("INSERT relatorios_arquivos SET upag='$upag', tipo='recesso', maquina='http://" . $_SERVER['SERVER_ADDR'] . "/sisref/relatorios/', arquivo='$arquivo', data_arquivo='$data_hoje' ");
}
else
{
