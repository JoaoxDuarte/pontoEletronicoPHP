...
if (empty($senhaatual))
{
    $msg_erro .= ". Senha utilizada atualmente não informada!\n";
}
else if (strlen($senhaatual) < 6)
{
    $msg_erro .= ". É obrigatório informar a senha utilizada atualmente!\n";
}

if (empty($senhanova))
{
    $msg_erro .= ". NOVA Senha não informada!\n";
}
else if (strlen($senhanova) < 6)
{
    $msg_erro .= ". A NOVA Senha é obrigatória e deve ter 8 caracteres alfanuméricos!\n";
}
else if ($senhaatual == $senhanova)
{
    $msg_erro .= ". A NOVA Senha deverá ser diferente da senha utilizada atualmente!\n";
}

if (empty($senhanova_confirmar))
{
    $msg_erro .= ". Senha de confirmação não informada!\n";
}
else if (strlen($senhanova_confirmar) < 8)
{
    $msg_erro .= ". A Senha de confirmação é obrigatória e deve ter 8 caracteres alfanuméricos!\n";
}
else if ($senhanova != $senhanova_confirmar)
{
    $msg_erro .= ". A Senha de confirmação não confere com a NOVA Senha!\n";
}
...
else
{
    ...
    $oDBase->query("SELECT * FROM usuarios WHERE siape = :siape AND senha = :senha ", array(
        array(':siape', $lSiape, PDO::PARAM_STR),
        array(':senha', $senhaatual, PDO::PARAM_STR)
        )
    );
    ...
    if ($numrows > 0)
    {
        ...
        $oDBase->query("UPDATE usuarios SET senha = :senha, prazo='0' WHERE siape = :siape ", array(
            array(':senha', $senhanova, PDO::PARAM_STR),
            array(':siape', $lSiape, PDO::PARAM_STR)
            )
        );
