...
function htmlCabecalho()
{
    ...
    <script>
        ...
        var mes  = '<?= $mes; ?>';
        var upag = '<?= $upag; ?>';
        var codigos_ocorrencia = "<?= $codigos_ocorrencia; ?>";
        ...
        function startTask()
        {
            ...
            var filtrar = "ano=" + ano + "&mes=" + mes + "&codigos_ocorrencia=" + codigos_ocorrencia + "&upag=" + upag;

            if (!!window.EventSource)
            {
                source = new EventSource("sisref_relatorio_comando_gerar_txt_por_ocorrencia.php?" + filtrar);
                ...
                source.addEventListener("message", function(e)
                {
                    ...
                    if(e.lastEventId == 'CLOSE')
                    {
                        ...
                        if (registros !== 0)
                        {
                            ...
                            tempLink.setAttribute('download', "siapenet.txt");
                            tempLink.click();

                            $.ajax({
                                "url": "sisref_relatorio_comando_gerar_txt_por_ocorrencia.php",
                                "method": "POST",
    ...
    <div class="container" style='padding-bottom:30px;margin-left:0%;'>
        ...
        <div align="left">
            <button id="exportar-csv" class="btn btn-default btn_adicionar" style="padding-left:30px;padding-right:30px; margin-bottom: 4px;"><span class="glyphicon glyphicon-download-alt" aria-hidden="true"></span> Gerar TXT para SIAPENET</button>
