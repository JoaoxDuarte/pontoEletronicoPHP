/*
 Milonic DHTML Website Navigation Menu - Version 3.5.01
...
function fixb(mnu)
{
    ...
    if (menu[11])
    {
        ...
        for (y = el - ic; y < el; y++)
        {
            ...
            hsep = gmobj("hsep" + y);
            if (y < el - 1)
                spos(hsep, null, typ[3], typ[2]);

            else
                spos(hsep, null, 0, 0, 0);
        }
        tw = lf - lfL;
        th = typ[2];
...
{
    if ((loadWait && MLoaded == 0) || inDragMode)
        return;
    subfound = 0;
    nv1 = v1;
    for (a = 1; a < menus; a++)
    ...
    clearTimeout(Mtimer);
    if (ParentMenu)
        ParentMenu.clearTimeout(ParentMenu.Mtimer);
    if (ns4)
    {
    ...
    if (SoImG[el])
    {
        ...
        if (ns4)
        {
            stimo = _d.layers[lyr].document.layers.eval("el" + el).document.layers.eval(stimo)
        }
        else
        {
            stimo = gmobj(stimo)
        ...
        if (frt > 0 && v1.indexOf("show-menu=") > -1)
        {
            if (lt)
            {
                ...
                if (pf.MLoaded)
                {
                    ...
                    tp = gpos(pf.mn)
                    if (fTop + tp[2] > (pf.sy + pf.wh))
                        fTop = pf.wh - tp[2] + pf.sy
...
function hl()
{
    ...
    if (arg[1].substring(0, 5) == "show-" && (!menu[11] || menu[6][16]) && (menu[11] || menu[6][10]))
    {
        ...
        {
            if (!parseLink(arg[1], "sourceframe"))
                imgact = "onmouseover=\"gmobj('el" + el + "').onmouseover();\" onmouseout=\"gmobj('el" + el + "').onmouseout();\"";
    ...
    if (hlnk.indexOf(" ") > 0)
    {
        ...
        if (hlnk.indexOf(" ") > 0)
            hlnk = hlnk.substr(0, hlnk.indexOf(" "));
    }
    if (ns4 && ofb == "transparent")
        ofb = ""
    if (!ofb && (mac || opera))
        ofb = "transparent"
    if (arg[7] + "" == "undefined")
    ...
    {
        if (bimg)
            bimg = " background=" + bimg;
        if (sbimg)
        {
            if (sbimg != "NONE")
                sbimg = " background=" + sbimg;
            else
                sbimg = ""
        }
        else
    ...
    else
    {
        ...
        if (opera && menu[3])
            dw = "width:" + (menu[3] - (arg[2][9] * 2) - (menu[4] * 2)) + "px;"
        if (!menu[3] && ie4)
            dw = "width:1px";
        if (menu[11] && opera)
        {
        ...
        if (menu[11] && menu[3] > 0)
            dw = "position:absolute;width:" + menu[3] + "px"
        hms = "";
        if (menu[11])
            hms = "<div style=\";top:0;position:absolute;\"><div onmouseover=\"clearTimeout(Mtimer)\" id=hsep" + el + " style=\"position:absolute;background:" + sbgc + ";width:" + menu[b] + "px; clip:rect(0 " + menu[b] + "px 100% 0);\">\n</div>\n</div>";
...
function dmenu(mnu)
{
    ...
    if (menu[14])
    {
        ...
        if (_d.layers)
            _d.write("<layer id=menuback" + mnu + " top=" + (menu[1] - m14h) + " height=2 width=" + m14w + " left=0 bgcolor=" + hlp[1] + "></layer>");
        else
    ...
    else
    {
        ...
        if (hlp[11])
            ...
            brd = " border:solid " + menu[4] + "px " + hlp[4] + ";";
        mt = "";
        scrof = "";
        if (!ns6)
            scrof = "overflow:hidden;";
        dmleft = 0;
        if (menu[2])
            dmleft = menu[2];
        if (ie55)
            mt = "<IFRAME frameborder=0 id=if" + mnu + " src=\"\" scroll=none style=\"FILTER:progid:DXImageTransform.Microsoft.Alpha(style=0,opacity=0);visibility:hidden;height:20;position:absolute;width:" + (menu[3] + ns6w) + "px;left:" + dmleft + "px;top:" + menu[1] + "px;z-index:5\"></iframe>";
    ...
    dlft = 0
    aleft = 0;
    if (menu[3] && menu[11] && (ns6 || opera))
        menu[3] = menu[3] - (hlp[9] * 2)
