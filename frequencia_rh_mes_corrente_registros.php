...
$_SESSION['rh_mes_corrente_registros'] = $dadosorigem;

// competencia
$dia         = '01';
$ano         = date('Y');
$mes         = date('m');
$competencia = date('mY'); // mes e ano, ex.: 032010
...
$dt_adm = $oDadosServidores->getDataAdmissaoDoServidor($mat);

$datasys = new trata_datasys();

$dias_no_mes = date('d'); //dataDia(subtrai_dias_da_data(date('d'), 1)); //numero_dias_do_mes($mes, $ano);

inserir_dias_sem_frequencia($mat, $dias_no_mes, $mes, $ano, $jornada, $lot, $nome_do_arquivo, $pm->dt_adm);
...
while ($pm_partners = $oDBase->fetch_object())
{
    ...
    if (in_array($pm_partners->oco, $codigoRegistroParcialPadrao)) //$pm_partners->oco == $CodigoRegistroParcialPadrao[0])
    {
        $ocorrencias_88888++;
    }

    if (in_array($pm_partners->oco, $CodigoSemFrequenciaPadrao)) //$pm_partners->oco == $CodigoSemFrequenciaPadrao[0])
    {
        $ocorrencias_99999++;
    }
...
$titulo_pagina = 'Manutenção Frequência - Mês Corrente';
$title         = _SISTEMA_SIGLA_ . ' | ' . $titulo_pagina;

$acao_manutencao = "rh_mes_corrente";
...
$javascript[] = 'frequencia_rh_mes_corrente_registros.js';
 ...
 *        mês corrente e outros dados do servidor
 *
...
function acaoLinkFrequenciaAlterar($siape, $pm_partners)
{
    ...
    $frequencia_alterar = base64_encode($siape . ':|:' . $sNome . ':|:' . $pm_partners->dia . ':|:' . $pm_partners->oco . ':|:' . $lot . ':|:' . $pm_partners->idreg . ':|:2:|:' . $jnd . ':|:' . $cod_sitcad . ':|:rh_mes_corrente');
...
//    if ($dia != '' || $dias >= date('d/m/Y')) //$_SESSION['sRH'])
//    {
        $acao_excluir = "<a href=\"javascript:window.location.replace("
            . "'frequencia_excluir.php?dados="
            . base64_encode($siape . ":|:" . $pm_partners->dia . ':|:rh_mes_corrente')
