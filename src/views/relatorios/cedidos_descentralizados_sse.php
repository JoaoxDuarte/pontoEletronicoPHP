...
SELECT 
    ...
    und.upag, cad.cod_sitcad, tabsitcad.descsitcad, IF(IFNULL(homologados.homologado,'N') NOT IN ('V','S'),'N','S') AS homologado
FROM
    servativ AS cad
LEFT JOIN
    tabsetor AS und ON cad.cod_lot = und.codigo
LEFT JOIN
    homologados ON (cad.mat_siape = homologados.mat_siape) AND (homologados.compet = '".$ano.$mes."')
LEFT JOIN 
    tabsitcad ON cad.cod_sitcad = tabsitcad.codsitcad
WHERE
    cad.cod_sitcad IN ('08') 
...
SELECT 
    cad.mat_siape, cad.nome_serv AS nome_serv, cad.cod_lot, und.descricao, cad.jornada, cad.freqh, 
    und.upag, cad.cod_sitcad, CONCAT(tabsitcad.descsitcad,'\nEXERCICIO DESCENTRALIZADO') AS descsitcad, IF(IFNULL(homologados.homologado,'N') NOT IN ('V','S'),'N','S') AS homologado
FROM
    servativ AS cad
LEFT JOIN
    tabsetor AS und ON cad.cod_lot = und.codigo
...
ORDER BY
    IF(IFNULL(homologado,'N') NOT IN ('V','S'),1,2), nome_serv
" );
...
while (list($mat_siape, $nome_serv, $cod_lot, $descricao, $jornada, $freqh, $upag, $cod_sitcad, $descsitcad, $homologado) = $oDBase->fetch_array())
{
    ...
    //sleep(1); // segundos
	usleep(15000); // usleep(250000); // 2000 milésimos de segundos = 2 segundos
}
